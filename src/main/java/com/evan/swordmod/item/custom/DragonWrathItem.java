...
public class DragonWrathItem extends SwordItem {
    public DragonWrathItem(Tier tier, int damage, float atkspeed, Properties propertiesp) {
    ...
    private void shoot(Player player,Level level){
       ...
       DragonFireball fireball = new DragonFireball(level, player, x - player.getX(), y - player.getEyeY(), z - player.getZ());
        fireball.setPos(player.getX() + lookVec.x * 2, player.getEyeY() + lookVec.y * 2, player.getZ() + lookVec.z * 2);
        fireball.setDeltaMovement(lookVec.x * 1, lookVec.y * 1, lookVec.z * 1);
